...
// Mock IFSC lookup helper
const lookupIfsc = (ifsc: string): { bankName: string; branch: string } | null => {
  const code = ifsc.toUpperCase().trim()
  if (code.startsWith('HDFC')) {
    return { bankName: 'HDFC Bank', branch: 'Madurai Main - Tamil Nadu' }
  }
  if (code.startsWith('SBIN')) {
    return { bankName: 'State Bank of India', branch: 'Commercial Branch' }
  }
  if (code.startsWith('ICIC')) {
    return { bankName: 'ICICI Bank', branch: 'Corporate Hub' }
  }
  if (code.startsWith('UTIB') || code.startsWith('AXIS')) {
    return { bankName: 'Axis Bank', branch: 'Retail Branch' }
  }
  if (code.startsWith('KKBK')) {
    return { bankName: 'Kotak Mahindra Bank', branch: 'CBD Branch' }
  }
  if (code.startsWith('PUNB')) {
    return { bankName: 'Punjab National Bank', branch: 'Main Branch' }
  }
  if (code.startsWith('BARB')) {
    return { bankName: 'Bank of Baroda', branch: 'City Branch' }
  }
  return null
}
...
export const GSTBankDetails = ({
  ...
  const handleIfscChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (cleaned.length >= 4) {
      const match = lookupIfsc(cleaned)
      if (match) {
        if (!data.bankName) onChange('bankName', match.bankName)
        if (!data.branch) onChange('branch', match.branch)
      }
    }
  }
