...
const RETURN_OPTIONS = [
  {
    ...
    desc: "Outward supplies and the summary return — the usual pair",
    fee: 2500,
  },
  {
    id: "gstr1" as const,
    title: "GSTR-1 only",
    desc: "Outward supplies statement",
    fee: 1500,
  },
  {
    id: "nil" as const,
    title: "Nil return",
    desc: "No outward or inward supplies in the period",
    fee: 500,
  },
];

export const GSTFilingPeriod = ({
  initialData,
  onContinue,
  onCancel,
}: GSTFilingPeriodProps) => {
  const [financialYear, setFinancialYear] = useState(
    initialData?.financialYear || "FY 2026-27",
  );
  ...
  const [returnType, setReturnType] = useState<"combo" | "gstr1" | "nil" | "">(
  ...
  const baseFee = returnType
    ? (RETURN_OPTIONS.find((opt) => opt.id === returnType)?.fee ?? 0)
    : 0;
  ...
  const handleProceed = () => {
    ...
    if (!selectedMonth) {
      newErrors.month =
        "Please select a filing month (e.g. August 2026) to continue";
    }
    if (!returnType) {
  ...
  return (
    <div className="gst-period-wrapper">
      <header className="gst-period-header">
        ...
        <p className="gst-period-header__subtitle">
          Pick the GSTIN and the period you want filed.
        </p>
      </header>
      ...
      <div className="gst-period-layout">
        <main className="gst-period-main">
          ...
          <section className="gst-period-card">
            ...
            <p className="gst-period-card__subtitle">
              You have one registration on this account.
            </p>

            <div
              className="gst-period-gstin-card"
              role="radio"
              aria-checked="true"
            >
              <div className="gst-period-gstin-card__left">
                <div
                  className="gst-period-gstin-card__icon-box"
                  aria-hidden="true"
                >
                  <svg
                    viewBox="0 0 24 24"
                ...
                <div className="gst-period-gstin-card__meta">
                  <span className="gst-period-gstin-card__number">
                    27AXTPD4419K1ZP
                  </span>
                  <span className="gst-period-gstin-card__details">
                    Shree Deshmukh Traders · Maharashtra · Regular scheme
                  </span>
                </div>
              </div>
          ...
          <section
            className={`gst-period-card ${errors.month ? "gst-period-card--has-error" : ""}`}
          >
            <h2 className="gst-period-card__title">Period</h2>
            <p className="gst-period-card__subtitle">
              Filings are current through July 2026.
            </p>

            <div className="gst-period-filters">
              <div className="gst-period-filter-field">
                <label htmlFor="period-fy" className="gst-period-filter-label">
                  Financial year
                </label>
                <div className="gst-period-select-wrap">
                  <select
                    id="period-fy"
                    className="gst-period-select"
                    value={financialYear}
              ...
              <div className="gst-period-filter-field">
                <label
                  htmlFor="period-freq"
                  className="gst-period-filter-label"
                >
                  Frequency
                </label>
                <div className="gst-period-select-wrap">
                  <select
                    id="period-freq"
                    className="gst-period-select gst-period-select--active"
                    value={frequency}
              ...
              {MONTH_FILING_CARDS.map((card) => {
                ...
                return (
                  <div
                    ...
                    onClick={() =>
                      !isLocked && handleSelectMonth(card.fullName)
                    }
                    role="button"
                  ...
                  >
                    <div className="gst-period-card-item__body">
                      {card.status === "active" ? (
                        <>
                          <div className="gst-period-card-item__title-single">
                            {card.fullName}
                          </div>
                          <div
                            className={`gst-period-card-item__subtext ${isSelected ? "gst-period-card-item__subtext--active" : ""}`}
                          >
                            {card.subText}
                          </div>
                        </>
                        ...
                        <>
                          ...
                          <div className="gst-period-card-item__subtext">
                            {card.subText}
                          </div>
                        </>
                      )}
                    ...
                    <div className="gst-period-card-item__indicator">
                      {isSelected && card.status === "active" && (
                        <div
                          className="gst-period-card-item__check"
                          aria-label="Selected"
                        >
                          <svg
                            viewBox="0 0 24 24"
          ...
          <section
            className={`gst-period-card ${errors.returnType ? "gst-period-card--has-error" : ""}`}
          >
                ...
                return (
                  ...
                  >
                    <div className="gst-period-return-item__left">
                      <div
                        className="gst-period-return-item__icon-box"
                        aria-hidden="true"
                      >
                        <svg
                          viewBox="0 0 24 24"
                      ...
                      <div className="gst-period-return-item__content">
                        <span className="gst-period-return-item__title">
                          {opt.title}
                        </span>
                        <span className="gst-period-return-item__desc">
                          {opt.desc}
                        </span>
                      </div>
                    </div>
                    {isSelected ? (
                      <div
                        className="gst-period-return-item__radio-check"
                        aria-hidden="true"
                      >
                        <svg
                          viewBox="0 0 24 24"
                      ...
                      <div
                        className="gst-period-return-item__radio-empty"
                        aria-hidden="true"
                      />
                    )}
                  </div>
          ...
          <div className="gst-period-actions">
            <button
              type="button"
              className="gst-period-btn-cancel"
              onClick={onCancel}
            >
              Cancel
            </button>
        ...
        <aside className="gst-period-sidebar">
          <div className="gst-period-order-card">
            ...
            <div className="gst-period-order-card__table">
              <div className="gst-period-order-card__row">
                <span className="gst-period-order-card__label">
                  {selectedMonth
                    ? `GST Filing — ${selectedMonth.split(" ")[0].substring(0, 3)} ${selectedMonth.split(" ")[1] || ""}`
                    : "GST Filing"}
                </span>
                <span className="gst-period-order-card__value">
              ...
              <div className="gst-period-order-card__row gst-period-order-card__row--total">
                <span className="gst-period-order-card__total-label">
                  Total payable
                </span>
                <span className="gst-period-order-card__total-amount">
            ...
            <div className="gst-period-order-card__delivery">
              <svg
                ...
                strokeLinecap="round"
                strokeLinejoin="round"
                className="gst-period-clock-icon"
              >
          ...
          <div className="gst-period-deadline-box">
            <div
              className="gst-period-deadline-box__icon-box"
              aria-hidden="true"
            >
              <svg
                viewBox="0 0 24 24"
            ...
            <div className="gst-period-deadline-box__content">
              <h4 className="gst-period-deadline-box__title">
                18 days to the deadline
              </h4>
              <p className="gst-period-deadline-box__subtitle">
                GSTR-3B due 20 Sep 2026
              </p>
            </div>
          </div>
        </aside>
